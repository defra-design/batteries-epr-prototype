...
export const runRegulatorOperatorList = (doc, loc) => {
  ...
  const operators = storage
    .listOperators()
    .filter((o) => o.agencyCode === agency?.code)
  ...
  body.innerHTML = operators
    ...
    .map((op) => {
      ...
      const statusText =
        payload.copy.statuses[op.approvalStatus] ?? op.approvalStatus
      const tagClass = STATUS_TAG_CLASSES[op.approvalStatus] ?? ''
      const typeLabel =
        payload.copy.typeLabels[op.approvalType] ?? op.approvalType
