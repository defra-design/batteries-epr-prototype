...
describe('initIaConfirmation', () => {
  test('returns true when authenticated', () => {
    ...
    expect(initIaConfirmation(document, globalThis.location)).toBe(true)
  })

  test('redirects to /sign-in when no user', () => {
    expect(initIaConfirmation(document, globalThis.location)).toBe(false)
  ...
  test('redirects to scheme-represented when registration is on the scheme route', () => {
    ...
    expect(initIaConfirmation(document, globalThis.location)).toBe(
