...
describe('shared prototype members-list add-member helpers', () => {
  test('basePageModel carries the compliance scheme service name and an empty navigation', () => {
  ...
  test('buildHydrationPayload composes the standard hydrate payload', () => {
    expect(buildHydrationPayload('organisationType')).toEqual({
  ...
  test('buildHydrationPayload sets skipHydration when requested', () => {
    expect(
      buildHydrationPayload('organisationType', { skipHydration: true })
    ).toEqual({
      step: 'organisationType',
      target: 'hydrate',
      skipHydration: true
  ...
  test('buildStepPayload includes the next step path for non-terminal steps', () => {
    expect(
      buildStepPayload('legalNoticesAddress', 'draft', { x: 1 }).nextStep
    ).toBe(paths.prototypeMembersListAddMemberAppropriatePerson)
  })

  test('buildStepPayload yields nextStep null for the last step', () => {
    expect(
      buildStepPayload('checkAnswers', 'draft', { x: 1 }).nextStep
    ).toBeNull()
  })

  test('buildStepPayload uses the override path when provided', () => {
    expect(
      buildStepPayload('organisationType', 'draft', { x: 1 }, '/prototype')
        .nextStep
    ).toBe('/prototype')
  })
  ...
  test('isAllowedReturn accepts only safe in-app paths', () => {
    expect(
      isAllowedReturn(paths.prototypeMembersListAddMemberOrganisationType)
    ).toBe(true)
