...
const renderStep = (h, request, step, viewModel) => {
  const application = content.complianceScheme(request).application
  ...
  return h.view(STEPS[step].view, {
    ...
    saveAndContinueAction: application.saveAndContinueAction,
...
export const applicationController = {
  ...
  post: {
    handler(request, h) {
      ...
      const { schema, fieldMessages, toSchemePatch } = STEPS[step]
      ...
      const patch = toSchemePatch(value)
